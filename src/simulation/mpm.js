...
export default class MPM {
  ...
  singleGridUpdate = ti.func((grid, I, mouse_position, click_strength) => {
    ...
    if (grid.grid_m[I] > 0) {
      ...
      grid.grid_v[I] +=
        (dist / (0.01 + ti.norm(dist))) * this.dt * click_strength
      // boundary handle
        ...
        for (let p of ti.range(material.n_particles)) {
          ...
          for (let i of ti.static(ti.range(3))) {
            for (let j of ti.static(ti.range(3))) {
              ...
              let weight =
                this.quadraticKernel((fx - offset)[0]) *
                this.quadraticKernel((fx - offset)[1])
              ...
              grid.grid_n[base + offset] +=
                weight * material.p_mass * (ti.f32(offset) - fx)
            }
          }
        }
      },
        ...
        for (let I of ti.ndrange(this.grid[0].n_grid, this.grid[0].n_grid)) {
          ...
          if (ti.static(this.grid.length > 1)) {
            ...
            if (m1 > 0.0 && m2 > 0.0) {
              ...
              let ni = ti.normalized(
                this.grid[0].grid_n[I] - this.grid[1].grid_n[I],
              ) // grid normal
    ...
    this.gridToParticle = ti.classKernel(
      ...
      (material, grid) => {
        for (let p of ti.range(material.n_particles)) {
          ...
          material.J[p] =
            (1.0 + this.dt * (new_C[0][0] + new_C[1][1])) * material.J[p]
          material.F[p] = (
            [
              [1.0, 0.0],
              [0.0, 1.0],
